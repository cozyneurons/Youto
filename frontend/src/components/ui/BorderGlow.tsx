...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  if (!match) return { h: 240, s: 80, l: 80 }; // Defaulting to a nice purple/blue if missing
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  const getEdgeProximity = useCallback((el: HTMLElement, x: number, y: number) => {
    ...
    // Smooth proximity for a nicer glow curve
    const prox = Math.min(Math.max(1 / Math.min(kx, ky), 0), 1);
    return Math.pow(prox, 1.5); // easing the proximity
  }, [getCenterOfElement]);
