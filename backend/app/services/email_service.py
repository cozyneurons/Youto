...
def _send_email(to_email: str, subject: str, html_body: str) -> None:
    if not settings.SMTP_HOST or not settings.SMTP_PORT:
        logger.warning("[EMAIL SKIP] SMTP not configured. Would have sent '%s' to %s", subject, _redact_email(to_email))
        return
...
def send_welcome_email(email: str, name: str) -> None:
    ...
    <html>
      <body>
        <h2>Welcome to Youto, {safe_name}!</h2>
...
def send_course_completed(email: str, course_title: str) -> None:
    ...
    <html>
      <body>
        <h2>You did it!</h2>
        <p>You have successfully completed the course: <strong>{safe_title}</strong>.</p>
